...
func main() {
	...
	discovery := fs.String("discovery", "", "")
	logDebug := fs.Bool("log-debug", true, "log debug-level information")
	logTimestamps := fs.Bool("log-timestamps", true, "prefix log lines with timestamps")
...
func NewClientHandler(c *oidc.Client, issuer string, cbURL url.URL) http.Handler {
	...
	mux.HandleFunc("/loginWithOfflineAccess", handleLoginFuncWithOfflineAccess(c))
	mux.HandleFunc("/register", handleRegisterFunc(c))
	mux.HandleFunc("/refresh_token", handleRefreshTokenFunc(c))
...
func handleIndex(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte("<br>"))
	w.Write([]byte("<a href='/loginWithOfflineAccess'>login with offline_access</a>"))
	w.Write([]byte("<br>"))
...
func handleLoginFuncWithOfflineAccess(c *oidc.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		oac, err := c.OAuthClient()
		if err != nil {
			panic("unable to proceed")
		...
		u, err := url.Parse(oac.AuthCodeURL("", "offline", ""))
		if err != nil {
			panic("unable to proceed")
		}
...
func handleRefreshTokenFunc(c *oidc.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {

		refresh_token := r.FormValue("refresh_token")
		log.Infof("Refresh token received: %v", refresh_token)

		jwt, err := c.RefreshToken(refresh_token)
		if err != nil {
			log.Fatalf("There was an error retrieving refresh token:", err)
			panic("unable to proceed")
		}

		claims, err := jwt.Claims()
		if err != nil {
			log.Fatalf("There was an error retrieving claims:", err)
			panic("unable to proceed")
		}
		...
		<br>
		<a href="/refresh_token?refresh_token=%s">Retrieve refresh token</a>
		</body>
		</html>`,
			jwt.Encode(),
			refresh_token,
...
func handleCallbackFunc(c *oidc.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if code == "" {
			s := fmt.Sprintf("code query param must be set. Query values: %v", r.URL)
			phttp.WriteError(w, http.StatusBadRequest, s)
			return
		}

		oidclient, _ := c.OAuthClient()
		tokenResponse, err := oidclient.RequestToken(oauth2.GrantTypeAuthCode, code)
		if err != nil {
			phttp.WriteError(w, http.StatusBadRequest, err.Error())
			return
		}

		jwt, err2 := jose.ParseJWT(tokenResponse.IDToken)
		if err2 != nil {
			phttp.WriteError(w, http.StatusBadRequest, err2.Error())
			return
		}
